...
class add_quiz_skill extends core_skill_base implements skill_trigger_provider_interface {
    ...
    /**
     * Can create the quiz in a course other than the current one (cross-context).
     *
     * @return bool
     */
    ...
    private function build_source_clarification(array $categories, string $lead = ''): preflight_result_v2 {
        $lines = [$lead !== '' ? $lead : 'Which questions should I add to the quiz?', ''];
        ...
        return $this->clarify(implode("\n", $lines), 'ADD_QUIZ_QUESTION_SOURCE', $options);
    }
