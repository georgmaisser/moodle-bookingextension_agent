# WBAgent Workflow Uebersicht (bookingextension_agent)
...
```mermaid
flowchart TD
    A[User tippt Nachricht im Textfeld] --> B[AMD bookingextension_agent/aiinstructions.js sendMessage]
    B --> C[WS bookingextension_agent_ai_send_message]
    ...
    N --> O[WS bookingextension_agent_ai_confirm_run]
    O --> P{aiexecutionmode}
    P -->|direct| M
    ...
    U4 --> V
    N --> W[WS bookingextension_agent_ai_render_command_preview]
    W --> V
    O --> V
    V --> Y[WS bookingextension_agent_ai_poll_thread fuer Planner-Schritte]
```

## Wie fuehrt der Agent Aufgaben aus?

1. Das Frontend ruft bookingextension_agent_ai_send_message auf.
...
- Die zweite Confirmation entsteht nicht im Erst-Interpreterlauf, sondern nach einem Execute-Fehler in bookingextension_agent_ai_confirm_run.
...
### Vom Chat-UI genutzte bookingextension_agent Webservice-Funktionen
...
- bookingextension_agent\\external\\search_users::execute (in-process aufgerufen)
- bookingextension_agent\\external\\search_courses::execute (in-process aufgerufen)
