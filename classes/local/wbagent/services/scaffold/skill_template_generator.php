...
namespace bookingextension_agent\local\wbagent\services\scaffold;
...
class skill_template_generator {
    ...
    public static function generate(array $spec): array {
        ...
        $relativepath = 'classes/local/wbagent/' . $spec['domain'] . '/skills/' . $spec['classfile'];
    ...
    private static function build_skill_php(array $spec): string {
        $namespace = $spec['namespaceroot'] . '\\local\\wbagent\\' . $spec['domain'] . '\\skills';
        $useprefix = 'bookingextension_agent\\local\\wbagent';
...
PHP;
    ...
    private static function build_readme(array $spec, string $relativepath): string {
        ...
        return "# Skill template: {$spec['skillname']}\n"
            ...
            . "The agent auto-discovers any skill under `classes/local/wbagent/<domain>/skills/` - **no**\n"
