 ...
 * Selector must still produce the correct task; retry handling is infrastructure-level.
...
class retry_preflight_recovery extends abstract_benchmark_scenario {
    ...
    public function get_description(): string {
        return 'Selector picks mutation task after simulated preflight transient error context';
    }
    ...
    public function assert_additional(array $result): array {
        return [
            [
                'label'  => 'Correct task selected despite error context in message',
                'passed' => ($result['commands'][0]['task'] ?? '') === 'mod_booking.book_users',
                'detail' => 'task: ' . ($result['commands'][0]['task'] ?? 'none'),
            ],
        ];
    }
}
