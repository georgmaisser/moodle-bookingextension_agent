...
class benchmark_db_writer {
     ...
     *
     * @param array $rundata    Fields for local_wbagent_benchmark_runs.
    ...
    public function write_run(array $rundata, array $scenarios, array $metrics): int {
        ...
        $runid = $DB->insert_record('local_wbagent_benchmark_runs', $runrecord);

        foreach ($scenarios as $scenario) {
            $DB->insert_record('local_wbagent_benchmark_scenarios', (object) array_merge([
                'run_id'                  => $runid,
                'scenario_key'            => '',
                'scenario_class'          => '',
                'passed'                  => 0,
        ...
        foreach ($metrics as $metric) {
            $DB->insert_record('local_wbagent_benchmark_metrics', (object) [
                'run_id'         => $runid,
    ...
    public function pin_baseline(int $runid, string $label, string $description = '', int $createdby = 0): int {
        global $DB;
        $DB->set_field('local_wbagent_benchmark_runs', 'is_baseline', 1, ['id' => $runid]);
        return $DB->insert_record('local_wbagent_benchmark_baselines', (object) [
            'run_id'      => $runid,
            'label'       => $label,
            'locked'      => 0,
    ...
    public function get_latest_baseline(): ?\stdClass {
        global $DB;
        $sql = 'SELECT r.* FROM {local_wbagent_benchmark_runs} r
                  JOIN {local_wbagent_benchmark_baselines} b ON b.run_id = r.id
    ...
    public function purge_old_runs(int $days = 365): int {
        ...
        $sql = 'SELECT id FROM {local_wbagent_benchmark_runs}
        ...
        $DB->delete_records_select('local_wbagent_benchmark_scenarios', "run_id {$insql}", $params);
        $DB->delete_records_select('local_wbagent_benchmark_metrics', "run_id {$insql}", $params);
        $DB->delete_records_select('local_wbagent_benchmark_runs', "id {$insql}", $params);
        return count($ids);
    }
