...
- [!] doppelte catch-Blöcke (`:332–407`/`:574–657`) auf einen Pfad — **KEIN echtes Duplikat:** Der catch klassifiziert mit `infer_from_issue_codes([])` → `''`, und `''` ∉ `RETRYABLE_ERROR_CLASSES` (`['provider_timeout','transient_io']`) → der retryable-Zweig im catch (build_retry_decision/to_retry_waiting) ist **toter Code**; Exceptions gehen immer auf `to_failed('EXECUTION_EXCEPTION_FATAL')`. Der In-try-Fail-Pfad retried dagegen echt. Merge wäre entweder no-op oder Dead-Code-Removal → **Georg-Entscheidung** (Policy: nicht eigenmächtig angleichen). War der tote Zweig als künftiger Hook gedacht?
