...
- [ ] `agent_runtime::run_followup_loop()` extrahieren, `confirm()` ruft es
- [ ] `confirm()` → `resolve_run_target` / `execute_confirmed_run` / `classify_execution_outcome` / `build_followup_response`
- [ ] doppelte catch-Blöcke (`:332–407`/`:574–657`) auf einen Pfad
- [ ] bestehende confirm-run-Tests müssen unverändert grün bleiben (Verhaltens-Anker)
