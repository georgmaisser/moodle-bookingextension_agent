...
if ($runid <= 0) {
    $run = $GLOBALS['DB']->get_record_sql(
        'SELECT * FROM {bx_agent_benchmark_runs} ORDER BY timecreated DESC LIMIT 1'
    );
} else {
    $run = $GLOBALS['DB']->get_record('bx_agent_benchmark_runs', ['id' => $runid]);
}
...
$metrics = $GLOBALS['DB']->get_records('bx_agent_benchmark_metrics', ['run_id' => $run->id]);
$metricsmap = [];
...
if ($baseline) {
    $basemetrics = $GLOBALS['DB']->get_records('bx_agent_benchmark_metrics', ['run_id' => $baseline->id]);
    $baselinemap = [];
