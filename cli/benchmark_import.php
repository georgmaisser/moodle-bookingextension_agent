...
// Idempotency check.
if ($uuid !== '' && $DB->record_exists('local_wbagent_benchmark_runs', ['run_uuid' => $uuid])) {
...
$now = time();
$newrunid = $DB->insert_record('local_wbagent_benchmark_runs', (object)$rundata);
...
foreach ((array)$data['scenarios'] as $scenario) {
    ...
    $DB->insert_record('local_wbagent_benchmark_scenarios', (object)$s);
}
...
foreach ((array)$data['metrics'] as $metric) {
    ...
    $DB->insert_record('local_wbagent_benchmark_metrics', (object)$m);
}
...
foreach ((array)($data['baselines'] ?? []) as $baseline) {
    ...
    $DB->insert_record('local_wbagent_benchmark_baselines', (object)$b);
}
