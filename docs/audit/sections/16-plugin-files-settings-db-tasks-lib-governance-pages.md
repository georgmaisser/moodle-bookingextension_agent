...
> **✅ FIXED 2026-06-30** (commit `5c6556c`) — `db/upgrade.php` is now install-only (a no-op `return true;`); the dead `local_wizard_*` upgrade blocks were removed and `data-model-and-db.md` corrected (see C2-F02). Fresh installs build `bx_agent_*` from `install.xml`; no pre-rename install exists.
