...
> **✅ FIXED 2026-06-30 — see capability audit CAP-01 for the COMPLETE fix.** A first pass gated on `user_can_view_profile($user)`, but the capability-fidelity sub-audit ([`docs/audit/capability/`](../capability/README.md), CAP-01) proved that insufficient: `user_can_view_profile()` returns `true` for everyone when `$CFG->forceloginforprofiles` is off (common default) and does not gate identity fields. `execute()` now (a) drops any candidate the actor has no relationship with (self / shared course / site-level `user:viewdetails`/`viewalldetails` / admin) and (b) strips identity fields unless the actor holds `moodle/site:viewuseridentity` (system or shared course). Covered by `search_users_capability_test` (4/4, first `viewuseridentity` denial test).
...
### [12-F02] ✅ RESOLVED (was 🟡 MEDIUM) · D3 Structure · classes/local/wizard/wizard/skills/recreate_skill_catalog_skill.php
**✅ Resolved 2026-06-30 (scope part):** `get_schema()` now declares `'prompt_meta' => ['context_scopes' => ['system'], …]`, so the skill reports its real site-wide blast radius instead of the inherited `['module']` default. (`enrich_schema_with_prompt_meta` early-returns when `prompt_meta` is present; the auto-derived `input_fields_for_prompt`/`anchor_fields` were empty here anyway and are declared explicitly.) phpcs clean. **Capability move now done (capability audit CAP-03):** the per-skill capability was moved from `$teacherskills` to `$managerskills` (no longer teacher-grantable); gated by its manager-only name-derived capability (Gate 1), like the other `wizard.*` meta skills. `recreate_skill_catalog_capability_test` (3/3) locks it. — _Original finding below._
