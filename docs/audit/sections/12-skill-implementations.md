...
**✅ Resolved 2026-06-30 (scope part):** `get_schema()` now declares `'prompt_meta' => ['context_scopes' => ['system'], …]`, so the skill reports its real site-wide blast radius instead of the inherited `['module']` default. (`enrich_schema_with_prompt_meta` early-returns when `prompt_meta` is present; the auto-derived `input_fields_for_prompt`/`anchor_fields` were empty here anyway and are declared explicitly.) phpcs clean; no test asserts this skill's scope. **Left for a maintainer decision:** the audit's secondary suggestion to move the per-skill capability from teacher to `$managerskills` — that changes *who* may run it, a governance call. — _Original finding below._
