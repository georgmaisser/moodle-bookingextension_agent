...
### [15-F01] ✅ RESOLVED (was 🟠 HIGH) · D1 Security · classes/local/wizard/llm_debug_logger.php + services/llm/llm_call_service.php
> **✅ RESOLVED 2026-06-30 (gated + retention).** Maintainer decision (George): **log only when `aidebugmode` is on.** Both `llm_call_service` call sites now use `llm_debug_logger::log_exchange` (self-gates on `is_enabled()`); the bypass method `log_exchange_always` and the `$forcelog` parameter were **removed** so the gate cannot be circumvented — `bx_agent_ai_llm_debug` stays empty in normal operation. Retention (`cleanup_old_llm_debug_task` + `llm_debug_retention_days` 30d) still caps rows while debug mode is on. The real-LLM test base (`abstract_agent_testcase::setUp`) enables `aidebugmode` so its trail assertions hold. phpcs clean. — _Original finding below._
...
### [15-F02] ✅ RESOLVED (was 🟡 MEDIUM) · D1 Security · classes/external/store_provider_apikey.php + configure_provider_from_existing.php
> **✅ RESOLVED 2026-06-30.** Maintainer decision (George): a **dedicated capability**, admin-only by default, not `site:config`. New `bookingextension/agent:manageaiproviders` (`RISK_CONFIG`, `CONTEXT_SYSTEM`, **empty archetypes** → admin-only, no automatic role assignment, explicitly delegable — the `runbenchmarks` pattern). Both endpoints now `require_capability('…:manageaiproviders', …)` instead of the manager-grantable `requesttrial`. Lang strings (en/de) added; version bumped to register the cap. The trial-request endpoints (`request_trial_key`, `activate_trial_context`) keep `requesttrial`. — _Original finding below._
