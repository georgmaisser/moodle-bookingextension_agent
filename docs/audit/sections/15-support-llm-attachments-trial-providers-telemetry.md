...
> **✅ FIXED 2026-06-30 (retention route)** — new `cleanup_old_llm_debug_task` + `llm_debug_retention_days` (30d default) + `conversation_store::purge_old_llm_debug_entries()` bound the table. Always-on logging kept (test harness asserts on it / prod diagnostics); the `is_enabled()` gating option is left open for the maintainer. See `docs/audit/README.md` remediation log.
