...
### [02-F02] ✅ RESOLVED — doc fixed, method accepted (was 🟢 LOW) · D3 Structure · authorization_service.php
**✅ Resolved 2026-06-30:** the **false doc claim** that `require_capability_at` is "used by the runtime context switch (`context_resolver`)" was corrected in ch.02 §1 (now: "available helper; currently no runtime caller — Gate 2 is enforced via `native_capability_guard` in preflight"). **Decision: keep the method.** It is a small, correct, public method on the `agent_authorization_service` **interface** (the documented authorization contract); removing it is an interface break for no functional gain, and a future operating-context re-check is a plausible consumer. So the doc is now accurate and the dead-method concern is accepted/closed, not churned. — _Original finding below._
...
### [02-F04] ✅ RESOLVED (was 🟡 MEDIUM) · D6 Docs coverage · docs/architecture/02-authorization-and-context.md (whole chapter)
**✅ Resolved 2026-06-30:** the operating-context subsystem is now documented — `09-preflight-pipeline.md §2b` describes `skill_operating_context_resolver`, the scope cascade, `CONTEXT_TARGET_UNRESOLVED` + candidate clarification and `native_capability_guard` (Gate 2 at the operating context), and `02-authorization-and-context.md §2` adds the "ambient vs operating context" split (Gate-1-ambient / Gate-2-operating, `LG_OPCTX`). The full-access/readonly gate is now also documented: `02 §3b` covers `agent_access_service::has_full_access()` (WB-LLM endpoint check) and `wb_license` (PRO `wbagent`/`bookingagent`), tying to the executor's `DENY_REQUIRES_PRO`. — _Original finding below._
