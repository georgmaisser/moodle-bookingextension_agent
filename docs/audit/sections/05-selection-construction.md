...
### [05-F02] ✅ RESOLVED (was 🟡 MEDIUM) · D4 Duplication · parameter_constructor + interpreter
**✅ Resolved 2026-06-30:** the verbatim-duplicate `prune_empty_input_values()` was extracted into `services/input_payload_pruner::prune()` (the small shared helper the finding recommended). Both `parameter_constructor` and `interpreter` now call it; both private copies removed. Behaviour kept verbatim (drop blank strings/null, drop arrays that prune to empty, KEEP `0`/`false`, non-empty strings untrimmed, **keys preserved**) — proven by a standalone harness (6/6 byte-identical old-vs-helper across nulls, blanks, 0/false, nested empties, key-gap cases). `tests/input_payload_pruner_test.php` locks the semantics. php -l + phpcs clean. — _Original finding below._
