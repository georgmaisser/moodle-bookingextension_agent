...
- [x] **S5 — Skill-Agnostik herstellen (§5.C HIGH) · ERLEDIGT**
...
- [x] **S6 — `course_targeted_skill`-Trait + Diagnose-Foundation (§6-P1.11) · ERLEDIGT** (Commits `67e3181`, `cf3f670`)
      - [x] **Trait `course_targeted_skill`**: dedupt `supports_target_context()`+`get_target_selector()` aus 6 Skills. + `course_targeted_skill_trait_test`.
      - [x] **`diagnostic_result_builder`**: `row()`/Glyph/`error_result()` der 5 `diagnose_*` zentralisiert. + `diagnostic_result_builder_test`.
      - [x] **Trait `preflight_clarification`**: byte-identisches `clarify()` aus add/update_activity + add/update_quiz dedupt. **`quiz_question_service::build_source_clarification`**: der duplizierte Content-Builder aus add/update_quiz (Skill behält nur Lead+Code im dünnen Wrapper). + `quiz_source_clarification_test`. Volle Suite **539/539**.
