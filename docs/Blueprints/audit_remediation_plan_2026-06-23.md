...
- [~] **S5 — Skill-Agnostik herstellen (§5.C HIGH) · Risiko: mittel**
      - [x] **S5a ERLEDIGT** (Commit `25c9ba5`): Das skill-namens-spezifische Routing + die de/en-Keyword-Heuristik sind aus dem Orchestrator raus. Skills deklarieren `governance => ['mandatory_on_trigger' => true, 'intent_triggers' => [...]]` (Validator + Registry-Contract surfacen beides); eine **generische** Engine-Methode `ensure_trigger_mandatory_skills` injiziert jeden so-deklarierten Skill bei Trigger-Match — **keine** Skill-Namen/Sprach-Keywords mehr in der Engine. Die exakten Marker 1:1 in `explain_docs`/`list_skills` verschoben (verhaltenserhaltend). `doc_intent_routing_test` umgeschrieben (real-marker-getrieben) + neuer `mandatory_on_trigger_injection_test` (Flag gatet, no-flag wird nie injiziert, Validator-Extraktion). Volle Suite **525/525**.
      - [ ] **S5b offen**: `list_skills`/`search_skills` hinter einen von der Engine injizierten **Introspektions-/Discovery-Service** statt Registry/Evaluator/Embeddings-Services im Skill zu newen.
