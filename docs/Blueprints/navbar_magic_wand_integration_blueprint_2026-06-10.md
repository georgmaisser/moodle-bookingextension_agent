...
> **Status: umgesetzt** in `orchestrator::build_runtime_context_block()` + neue Helper-Methode `append_moodle_context_section()` (gefüttert aus `agent_context` + `get_fast_modinfo`, try/catch-gekapselt); `now_iso` jetzt minutengenau. Selection/Discovery unverändert schlank; voller Block bei `PHASE_PARAMETER_CONSTRUCTION` und Synchronizer (erkannt am `synchronization`-Memory-Channel). `booking_name:`-Key unverändert (Behavior-Erhalt). Doku: architecture/05 §3 (neue Tabelle), architecture/12 §3, Flowchart-Legende `LG_RCTX`. **Benchmark-Auflage (5.4.1) erfüllt — 2026-06-10 nachts, No-op-Isolation:** Lauf ID 11 (mit Block) und ID 12 (Block temporär neutralisiert) liefern **identische 8/15 (53,3%)** mit denselben 7 fehlschlagenden Szenarien — der Kontext-Block kostet keine Benchmark-Punkte. Die 7 Fails sind vorbestehende/parallele Baustellen: Clarification-/Confirm-Contract (Szenarien 5/7/8/9, bekanntes D2), veraltete Szenario-Erwartung vs. bewusstem search_skills-Fallback (11), Selektor-Ablenkung durch die heute neu registrierten Skills `diagnose_user_booking`/`search_courses` (4/13). Die vier Review-Korrekturen (Phasen-Namen, Caching-Argument, active_page_blocks gestrichen, agent_context als Datenquelle) sind eingearbeitet.
