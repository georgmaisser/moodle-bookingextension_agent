# wbagent → eigenständiges `local`-Plugin & Context-Unabhängigkeit — Vorbereitungsanalyse

**Datum:** 2026-06-08
**Status:** Reine Analyse (keine Code-Änderung)
**Ziel der Analyse:** Bestandsaufnahme, wie der wbagent aktuell `contextid` vs. `cmid` behandelt, wo noch harte Abhängigkeiten von `mod_booking` bestehen, und wo die Context-Unabhängigkeit (z. B. Course-Ebene, Nav-Bar, beliebige Moodle-Kontexte) heute **nicht** gegeben ist.
...
- **Gute Nachricht:** Das **Persistenz-/Scope-Modell ist bereits context-basiert** (`contextid` ist der Schlüssel für Threads/Runs/Messages/Debug), die DB-Tabellen heißen schon `local_wbagent_ai_*`, und die **Skill-Discovery ist bereits komponenten-agnostisch** (Provider-Modell, scannt *alle* Plugins). Es gibt sogar schon einen Nicht-Booking-Provider: **`local_entities`**.
...
|---|---|---|
| `local_wbagent_ai_threads` | `contextid` (Z. 11) **+ `bookingid` NOT NULL** (Z. 12) | context-basiert, aber booking-Spalte |
| `local_wbagent_ai_messages` | `threadid` (Z. 38, FK) | agnostisch |
| `local_wbagent_ai_runs` | `threadid`, `contextid` (Z. 50) | agnostisch |
| `local_wbagent_ai_llm_debug` | `threadid`, `contextid` (Z. 73) | agnostisch |
...
- `conversation_store::get_active_thread(userid, contextid)` und `get_or_create_thread(userid, contextid, bookingid)` → Thread-Lookup per **`contextid`** (gut), aber Erstellung verlangt eine **`bookingid`** (Kopplung).
  - Datei: `classes/local/wbagent/conversation_store.php:64, 84, 119`.
...
- `agent_runtime::resolve_cmid_from_contextid()` — wirft `coding_exception('Invalid module context id.')`, wenn der Context **kein** `context_module` ist; gibt sonst `$ctx->instanceid` (cmid) zurück.
  - `classes/local/wbagent/agent_runtime.php:261-271`.
- `orchestrator::process()` / `run_*_phase()` arbeiten durchgehend mit `cmid` und `context_module::instance($cmid)`.
- `executor::execute_commands()` löst `context::instance_by_id($contextid)` und erzwingt `context_module` (`classes/local/wbagent/executor.php:97-103`).
...
- `authorization_service::require_booking_module_context(int $contextid): context_module`
  `classes/local/wbagent/services/security/authorization_service.php:65-75`
...
- Für context-übergreifende Nutzung müsste die Capability auf eine breitere Ebene (z. B. `CONTEXT_COURSE`/`CONTEXT_SYSTEM` bzw. mehrere Levels) und unter `local/wbagent:*` neu definiert werden.
...
| `classes/local/wbagent/services/security/authorization_service.php` | 70 |
| `classes/local/wbagent/aiready.php` | 113 |
| `classes/local/wbagent/orchestrator.php` | 2174 (`build_runtime_context_block`) |
...
- `orchestrator::build_runtime_context_block()` → `$cm = get_coursemodule_from_id('booking', $cmid); $bookingname = format_string($cm->name)` und rendert `booking_name:` in den `SYSTEM_RUNTIME`-Block.
  `classes/local/wbagent/orchestrator.php:2174, 2133, 2137`.
...
- `aiready` nutzt `mod_booking\singleton_service`:
  `classes/local/wbagent/aiready.php:32` (use), `113` (`get_coursemodule_from_id('booking', …)`), `344` (`get_instance_of_booking_by_bookingid`), `361` (`get_instance_of_booking_option_settings`), `363` (`get_instance_of_booking_answers`).
...
|---|---|---|
| `classes/local/wbagent/services/lookup/option_lookup_service.php` | `mod_booking.search_options`, `mod_booking.update_option` | 67, 95 |
| `classes/local/wbagent/services/catalog/adaptive_skill_catalog_service.php` | `mod_booking.book_users`, `mod_booking.update_option_trainer` (ALWAYS_INCLUDE) | — |
| `classes/local/wbagent/services/phase_prompt_bundle_builder.php` | `mod_booking.create_option` | — |
| `classes/local/wbagent/orchestrator.php` | `mod_booking.create_option_canonical_fallback` | — |
| `classes/local/wbagent/benchmark/**` (Test-/Bench-Fixtures) | diverse `mod_booking.*` | — |

`skill_contract_validator.php:39` reserviert zudem `RESERVED_NAMESPACES = ['booking', 'core']`.

### 2.8 DB-Kopplung
- `local_wbagent_ai_threads.bookingid` ist `NOT NULL DEFAULT 0` (`db/install.xml:12`). Für ein booking-freies Plugin müsste die Spalte entfallen, nullable werden oder generisch (z. B. „domain scope id") umgedeutet werden.

---

## 2.9 Umgekehrte Kopplung: Skills → Engine (Dependency-Inversion nötig)

Nicht nur das Framework hängt an Booking — auch die **Skills hängen an der Engine**. Die Skill-Provider (`mod_booking/classes/local/wbagent` = 29 Dateien/246 Treffer; `local_entities` = 5 Dateien) importieren `bookingextension_agent\…`. Aufschlüsselung:
...
- **Provider-Modell (komponenten-agnostisch):** `skill_registry::make_default()` iteriert `core_component::get_component_names()` und lädt aus *jeder* Komponente `\{component}\local\wbagent\skill_provider`.
  `classes/local/wbagent/skill_registry.php` (make_default). Beleg: es existieren **drei** Provider:
  - `mod/booking/classes/local/wbagent/skill_provider.php`
  - `mod/booking/bookingextension/agent/classes/local/wbagent/skill_provider.php`
  - **`local/entities/classes/local/wbagent/skill_provider.php`** ← ein `local`-Plugin liefert bereits Skills.
- **Skill-Discovery:** `skill_discovery::get_skill_instances($component)` scannt `{plugindir}/classes/local/wbagent` generisch (`classes/local/wbagent/skill_discovery.php:44, 119-160`).
- **DB-Tabellen** heißen bereits `local_wbagent_ai_*` (kein `bookingextension_*`-Rename nötig).
...
**A. Plugin-Identität**
- Neuer Typ `local_wbagent` (Pfad `local/wbagent/`), Component `local_wbagent`, Namespace `local_wbagent\*` (mechanischer Massen-Rename von `bookingextension_agent\*`).
...
**C. Capability**
- Neue Capability `local/wbagent:useaiinstructions` mit passender/mehreren Contextlevels statt `CONTEXT_MODULE`.

**D. Datenmodell**
- `local_wbagent_ai_threads.bookingid` entfernen/nullable/umdeuten; `get_or_create_thread()`-Signatur entkoppeln (ohne `bookingid`).
...
- `classes/local/wbagent/services/security/authorization_service.php:65-122` — Booking-Modul-Gate (zentraler Blocker).
...
- `classes/local/wbagent/agent_runtime.php:261-271` — `resolve_cmid_from_contextid` (Modul-Zwang).
- `classes/local/wbagent/orchestrator.php:2174` — `booking_name` via `get_coursemodule_from_id('booking')`.
- `classes/local/wbagent/aiready.php:113, 344-363` — Booking-Readiness via `singleton_service`.
- `classes/local/wbagent/conversation_store.php:64, 84, 119` — Thread-Keying (contextid + bookingid).
- `db/install.xml:7-25` — `local_wbagent_ai_threads` inkl. `bookingid` (Z. 12).
- `classes/local/wbagent/skill_registry.php` — `make_default()` (komponenten-agnostische Provider-Discovery).
- `classes/local/wbagent/skill_discovery.php:44, 119-160` — generischer `local/wbagent`-Scan.
- `classes/local/wbagent/services/lookup/option_lookup_service.php:67, 95` — hardcodierte `mod_booking.*`-Skills im Framework.
