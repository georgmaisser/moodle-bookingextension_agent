# Auskopplung wizard → eigenständiges `local_wizard`-Plugin — Migrations-Blueprint

**Datum:** 2026-06-08
**Status:** Planung/Analyse (keine Code-Änderung)
**Vorgänger:** [wizard_local_plugin_context_decoupling_analysis_2026-06-08.md](wizard_local_plugin_context_decoupling_analysis_2026-06-08.md) (Context/Booking-Kopplungen)
**Ziel:** Den Agenten aus dem Booking-Subplugin `bookingextension_agent` in ein eigenständiges `local_wizard` überführen — per Search-Replace als Basis, mit klarer Liste aller manuellen Eingriffe, Parallelbetrieb-Strategie und Sync-Konzept.
...
1. **Ziel-Architektur: EIN Engine + EIN dünner Shim — nicht zwei Engines.**
   - `local_wizard` = die **Engine** (Runtime, Orchestrator, Skills-Framework, WS, Tabellen).
   - `bookingextension_agent` bleibt als **dünnes Booking-Subplugin** (nur die `bookingextension_interface`-Hooks: Option-Fields, Rules, col_actions, History) und **delegiert** an `local_wizard`. Kein Engine-Code mehr.
   - Damit ist „wenn beide installiert sind, wird nur wizard verwendet" **automatisch** erfüllt: es gibt nur eine Engine. Das ist der zentrale Gegenvorschlag zu „zwei vollständige Kopien synchron halten" (siehe §6/§7).
...
4. **Gleiche Tabellen für Parallelbetrieb ist machbar** (sie heißen bereits `local_wizard_ai_*`), erfordert aber eine bewusste **Tabellen-Ownership-Lösung** (nur ein Plugin deklariert sie; Adopt-Guards via `db/install.php`) — §5.
...
| DB-Tabellen (eigene install.xml) | **8 Tabellen, alle `local_wizard_*`**: `_ai_threads/_ai_messages/_ai_runs/_ai_llm_debug` + 4× `_benchmark_*` |
| **Externe Consumer des Namespace** | `mod_booking/classes/local/wizard` (**29 Dateien / 246 Treffer**), **`local_entities`** (5 Dateien), `mod/booking/view.php` (UI-Einbindung) |
...
Auf einer **Kopie** des Ordners nach `local/wizard/`:

1. **Namespace + Component-String:** `bookingextension_agent` → `local_wizard`
   (PHP-Namespaces, `get_string(..., 'local_wizard')`, `cache::make('local_wizard', …)`, WS-Funktionsnamen `local_wizard_ai_*`, Cache-Component, Event-Component).
2. **Capability/Slash-Form** (separater Lauf!): `bookingextension/agent` → `local/wizard`
   (Capabilities `local/wizard:useaiinstructions`, `require_capability`-Strings, Settings-Pfade).
3. **Lang-Dateien:** Inhalt der `$string[...]`-Keys bleibt; nur die **Dateinamen** `lang/en/bookingextension_agent.php` → `lang/en/local_wizard.php` (manuell), und alle `get_string`-Component-Args werden durch Lauf 1 erfasst.

**Reihenfolge:** zuerst Slash-Form (`bookingextension/agent`→`local/wizard`), dann Underscore-Form (`bookingextension_agent`→`local_wizard`). Sonst würde der zweite Lauf `bookingextension/agent` in `local_wizard/agent`-Artefakte zerlegen.
...
- [ ] **Ordner verschieben** `mod/booking/bookingextension/agent/` → `local/wizard/`.
- [ ] `version.php`: `$plugin->component = 'local_wizard'`; **`dependencies` auf `mod_booking` entfernen** (mod_booking wird optionaler Provider). `$plugin->maturity`/`requires` prüfen.
...
- [ ] Hardcodierte Pfade: `require($CFG->dirroot . '/mod/booking/bookingextension/agent/settings.php')` → `/local/wizard/settings.php` (in `agent.php`/Shim).

### 3.2 Datenbank / Tabellen-Ownership (Parallelbetrieb-kritisch — §5)
- [ ] Entscheiden, **wer die Tabellen besitzt**. Empfehlung: `local_wizard`. Dann muss der Booking-Shim die Tabellen aus *seiner* install.xml entfernen.
...
- [ ] `db/access.php`: `local/wizard:useaiinstructions` mit passendem `contextlevel` (nicht zwingend `CONTEXT_MODULE` — für course-/system-weite Nutzung breiter; siehe Context-Doc). Archetypes neu bewerten.
...
- [ ] **`mod_booking`** (29 Dateien/246 Treffer) und **`local_entities`** (5 Dateien): das sind **Skill-Provider**. Sie dürfen die Engine nicht referenzieren → Behandlung gemäß **§4** (Contract-Surface + Leak-Inversion), **nicht** per Search-Replace auf `local_wizard\…`.
- [ ] **`mod/booking/view.php`**: UI-Einbindung + WS-Funktionsname `bookingextension_agent_ai_*` → auf Shim oder `local_wizard_*` zeigen.
...
- Die Contract-Surface bleibt ein **Sub-Namespace innerhalb von `bookingextension_agent`** (z. B. `…\interfaces\*`, `…\dto\*`, `base_skill`). Kein `local_wizardsdk`-Plugin. Skills hängen an dieser Surface, nicht an Engine-Services.
...
**Dein Ziel:** beide installiert, gleiche Tabellen, faktisch nur wizard aktiv. **Bewertung: sinnvoll — mit einer Präzisierung.**
...
- Lösung: **`local_wizard` besitzt** `local_wizard_ai_*` (+ benchmark). Der Booking-Shim entfernt sie aus seiner install.xml.
- Für **Bestandsinstallationen** (Tabellen existieren bereits, vom alten Subplugin): `local_wizard/db/install.php` mit `table_exists()`-Adopt-Guards → keine Neuanlage, kein Datenverlust.
- **Uninstall:** der Shim darf die geteilten Tabellen nie droppen.

### 5.2 Runtime-Delegation („nur wizard wird verwendet")
- Mit dem **Engine+Shim-Modell ist das geschenkt**: der Shim *enthält* keine Engine, er ruft `local_wizard`. Es gibt nur einen Pfad.
- Falls (Übergang) der Shim doch noch eigenen Engine-Code hätte: einfache Feature-Detection `if (class_exists('\\local_wizard\\local\\wizard\\agent_runtime')) { delegiere } else { lokal }`. Das ist genau dein „automatisches Umstellen im Hintergrund" — aber es ist nur nötig, wenn man (unnötigerweise) zwei Engines hält.
...
- Der gewünschte Effekt („wizard ist die Engine") wird durch **Engine + dünner Shim** ohne diese Nachteile erreicht.
...
| Komponente | Zielort |
|---|---|
| Runtime, Orchestrator, Interpreter, Executor, Queue, Preflight, Synchronizer, Discovery/Embeddings, Core-Skills, WS-Endpoints, Tabellen, Caches, Tasks, AMD-UI | **`local_wizard`** |
| `bookingextension_interface`-Hooks (`contains_option_fields`, `get_option_fields_info_array`, `set_template_data_for_optionview`, `add_options_to_col_actions`, `get_allowedruleeventkeys`, `get_booking_history_description`) | **`bookingextension_agent`-Shim** (bleibt Booking-Subplugin), delegiert an `local_wizard` |
| Booking-Skills (`mod_booking.create_option`, `update_option`, `search_options`, …) | bleiben in **`mod_booking/classes/local/wizard`**, referenzieren künftig `local_wizard\…` (oder via Alias) |
...
Falls dennoch **zwei eigenständige Codebasen** gewünscht sind (z. B. der Booking-Subplugin soll auch **ohne** local_wizard voll funktionieren):

1. **Codegen/Mirror-Script + CI-Gate (am praktikabelsten bei Duplikat-Zwang):**
   - Kanonische Quelle = `local_wizard`.
   - Ein deterministisches Transform-Script erzeugt den Booking-Klon (reverse Search-Replace `local_wizard`→`bookingextension_agent`, Entfernen engine-fremder Teile).
...
2. **Git subtree** des Engine-Kerns in beide Plugins. Problem: die feste Namespace-Differenz (`local_wizard\` vs `bookingextension_agent\`) macht denselben Quelltext nicht teilbar, ohne Build-Zeit-Rewriting → unschön.
...
4. **Symlink** (Dev-only): `mod/booking/bookingextension/agent` → `local/wizard`. Nur lokal/dev, nicht deploybar, Namespace-Problem bleibt.
...
1. **PHPUnit-Reinit:** `php public/admin/tool/phpunit/cli/init.php` (neuer Component-Testsuite-Name `local_wizard_testsuite`).
...
4. **WS-Registrierung:** `php admin/cli/upgrade.php`; prüfen, dass `local_wizard_ai_*`-WS existieren und JS sie aufruft.
...
3. Skill-Provider importieren danach **nur noch** aus dem Contract-Namespace (Verifikation per `grep`: kein `…\\wizard\\(services|orchestrator|executor|conversation_store|privacy_anonymizer)` mehr in Skills).

**Phase 1 — Engine auskoppeln:**
4. **Branch + Kopie** des Ordners nach `local/wizard/` (Original vorerst belassen).
...
9. **Booking-Shim** umbauen: `bookingextension_agent` zu dünnem Forwarder (nur Interface-Hooks, delegiert an `local_wizard`); Tabellen aus dessen install.xml entfernen; Uninstall-Schutz; **keinen** eigenen skill_provider mehr registrieren (§10.2).
...
3. **Capability-Migration:** neue `local/wizard:*` ist auf Bestandsrollen nicht zugewiesen → bewusst Default-Archetypes setzen, sonst „permission denied".
...
8. **Gewählte Richtung „beide Plugins installiert": reversibler Cutover (B+)** (Georg, 2026-06-09 — umzusetzen erst, wenn local_wizard kommt, NICHT jetzt). Skill-Klassen sind an *einen* Engine-Namespace gebunden (`bookingextension_agent\…\skill_interface` ≠ `local_wizard\…\skill_interface`, verschiedene PHP-Typen). Solange nur `bookingextension_agent` ausgeliefert wird, ist das irrelevant — bei „nur Extension" bzw. „nur local_wizard" sind die jeweils anderen Skills schlicht nicht vorhanden (gewünschtes Verhalten). Relevant ist allein **beide gleichzeitig** (Upgrade-Pfad eines Booking-Kunden).
   - **Mechanik (Georgs Wunsch):** local_wizard wird *die* aktive Engine; `bookingextension_agent` stellt seine **Engine still** (Feature-Detection-Weiche: `class_exists('\local_wizard\…\agent_runtime')` → delegieren, selbst keine Aktion) + einmalige **Daten-Migration** in local_wizards getrennte Tables (§5). **Reversibel:** wird local_wizard deinstalliert, fällt die Weiche zurück und `bookingextension_agent` „erwacht" wieder. Solange wizard installiert ist, führt bookingextension keinerlei eigene Aktion aus.
   - **Konsequenz (unausweichlich):** Wenn Booking inaktiv ist, muss local_wizard die **Booking-Skills ausführen** (sonst verliert der Bestandskunde beim Install seine Booking-KI). Die Booking-Skills sind aber auf `bookingextension_agent`-Typen gebunden. Auflösung — zwei Optionen, Entscheidung fällt erst beim local_wizard-Cut:
     - **(empfohlen) Thin Bridge-Provider in `bookingextension_agent`:** stellt nur die *Engine* still, exponiert die Booking-Skills aber via Adapter, der `local_wizard\…\skill_interface` implementiert und an den echten Booking-Skill forwardet. Lädt nur bei vorhandenem local_wizard (optionale Kopplung). Wegen Codegen-Strukturgleichheit ~generischer Forwarder. SDK-frei, passt zu „bookingextension delegiert".
...
- Tabellen: `…/agent/db/install.xml` (8× `local_wizard_*`)
...
- Externe Consumer: `mod/booking/classes/local/wizard/**` (246), `local/entities/classes/local/wizard/**` (5), `mod/booking/view.php`
- Booking-Kopplungen (Detail): siehe [Context-Decoupling-Doc](wizard_local_plugin_context_decoupling_analysis_2026-06-08.md)
